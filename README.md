...
```sh
git clone https://github.com/ripple.mq/benchmark
cd benchmark
```
</details>
...
This section will guide you to setup a fully-functional local copy of `benchmark`
...
from [`benchmark's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `benchmark`, run the
command
...
Tests in `benchmark` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *benchmark*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *benchmark* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *benchmark* 	|     docker    	|
...
```text
benchmark_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
benchmark_v1.2.1_windows_x86_64.exe
```
...
To run `benchmark` in a docker container, read the instructions in
[docker section](./docker).

### Running `benchmark`

To run benchmark, use the command
