...
.PHONY: install
## `install`: Fetch dependencies needed to run `benchmark`
install:
...
.PHONY: run
## :
## `run`: Run `benchmark` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `benchmark` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `benchmark`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `benchmark`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
